# -*- coding: utf-8 -*-
"""
cart-pole swing-up experiment with rbf features and off-policy samples
"""
...
policy_tar = policies.MarcsPolicy(noise=np.array([.001]))


task = LinearContinuousValuePredictionTask(
    mdp, gamma, phi, theta0, policy=policy,
    target_policy=policy_tar,
...
alpha = .0001
lam = 0.2
lstd = td.RecursiveLSPELambda(lam=lam, alpha=alpha, eps=10, phi=phi, gamma=gamma)
...
name = "swingup_gauss_offpolicy"
title = "10. Cart-Pole Swinup Off-policy"
n_indep = 50
episodic = False
criterion = "RMSPBE"
