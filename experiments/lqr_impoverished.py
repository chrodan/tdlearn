...
params = itertools.product(list(np.arange(0.001, .01, 0.001)) + list(np.arange(0.01, 0.1, 0.01)), [0.01, 0.1, 0.5,1,2,4,8,16])
k = (delayed(run)(*p) for p in params)
res = Parallel(n_jobs=-1, verbose=11)(k)
import pickle
pickle.dump(dict(params=params, res=res), "TDC Errors")
